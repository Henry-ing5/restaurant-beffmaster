...
def obtener_cortes():
    try:
        conn = get_db()
        cursor = conn.cursor(dictionary=True)
        cursor.execute('SELECT nombre, descripcion, precio FROM cortes')
        cortes = cursor.fetchall()
        return jsonify(cortes), 200
    except Exception as e:
        logger.error(f"Error al obtener cortes: {str(e)}")
        return jsonify({"error": "Error al obtener cortes"}), 500

@app.route('/obtener-bebidas', methods=['GET'])
def obtener_bebidas():
    try:
        conn = get_db()
        cursor = conn.cursor(dictionary=True)
        cursor.execute('SELECT nombre, descripcion, precio FROM bebidas')
        bebidas = cursor.fetchall()
        return jsonify(bebidas), 200
    except Exception as e:
        logger.error(f"Error al obtener bebidas: {str(e)}")
        return jsonify({"error": "Error al obtener bebidas"}), 500
